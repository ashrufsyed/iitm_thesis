...
The present study has been carried out in OpenFOAM which is based on \cite{Weller1998}. The Lagrangian solver has two injection models based on the nature of injection source
viz. \texttt{pointInjection} model which injects the spray at a given point, and \texttt{detailed- SprayProfileInjection} model which injects the spray over a spherical sector of given injection radius. The configuration and experimental data to compare the spray statistics is taken from \cite{Zhou2015a}

Other sample references to check are \cite{Syed2015,Syed2018a,Sasidharan2017,Rajasekar2019}. One more reference, \cite{Roenby2016} with arxiv and doi.

More information on BIB\TeX\ is available in the book by
\cite{lamport:86}.  Now to cite the references within parentheses. There are many
references~\citep{lamport1:86,prabhu:xx} that explain how to use
